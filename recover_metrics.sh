...
echo "  bag/            -- rosbag with /explore/traversed_path (path length); /map"
echo "                     (own raw local SLAM map); /team_map_ddil (fused-from-peers"
echo "                     map, pre-local-merge); /nav_map (/map + /team_map_ddil"
echo "                     composite -- NOTE: silently falls back to local-only if"
echo "                     /team_map_ddil never arrived, so check /map vs /nav_map"
echo "                     vs /team_map_ddil together, don't trust /nav_map alone);"
echo "                     and the actual map traffic sent/received: /incoming/<peer>/map"
