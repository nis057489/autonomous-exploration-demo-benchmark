...
public:
  ...
  : Node("occupancy_grid_vxch_node")
  {
    ...
    // behavior). Defaults to disabled here -- any launch file that wants
    // debouncing must opt in explicitly by setting this parameter, so a
    // launch path nobody updated (e.g. a future one added the same way
    // hw_namespaced_stack.launch.py originally was) can't silently inherit
    // an 8s resend floor it was never meant to have. See
    // hw_namespaced_stack.launch.py / multi_robot_vxch_experiment.launch.py
    // for the actual opt-in default used on hardware/sim runs.
    min_resend_interval_s_ = declare_parameter<double>("min_resend_interval_s", 0.0);
