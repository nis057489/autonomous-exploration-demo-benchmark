...
private:
  void on_map(const nav_msgs::msg::OccupancyGrid::ConstSharedPtr & msg)
  {
    ...
    // No tiling here (single global pyramid over the whole grid) -- bands carry
    // no tile_row/tile_col metadata, so vxch_occupancy_grid_node's decoder falls
    // back to a single tile (0,0) covering the full grid. It still requires a
    // nonzero tile_size_cells to render at all, so set it to the grid width;
    // the actual value is irrelevant here since row0/col0 resolve to 0 either way.
    manifest.metadata["tile_size_cells"] = std::to_string(msg->info.width);
