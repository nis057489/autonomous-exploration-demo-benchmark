...
run_docker() {
  ...
  for _var in MAP_TRANSPORT CONTROLLER_TYPE BANDWIDTH_KBPS LOSS_PCT DELAY_MS HAAR_LEVELS COMPRESSION VARINT_ENCODING TILE_SIZE_M SCHEDULE_MODE RANDOM_SEED ROBOT_STARTUP_DELAY_S SPAWN_PRESET RECORD_METRICS IMPAIRMENT_MODE IMPAIRMENT_DELAY_S; do
    if [[ -n "${!_var:-}" ]]; then
